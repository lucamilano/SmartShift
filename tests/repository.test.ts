...
test('users cannot read/write another calendar or administer profiles/export', async () => {
  ...
  try {
    ...
    await assert.rejects(alice.deactivateProfile('bob'), /permessi/)
...
test('admins manage colleagues, disabling blocks an already-created repository and preserves history', async () => {
  ...
  try {
    ...
    await assert.rejects(admin.deactivateProfile('admin'), /tuo account/)
    await assert.rejects(admin.updateProfile('admin', '', '', 'user'), /tuo ruolo/)
    await admin.deactivateProfile('alice')
